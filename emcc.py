...
C_ENDINGS = ['.c', '.i']
CXX_ENDINGS = [
    '.cpp', '.cxx', '.cc', '.c++', '.CPP', '.CXX', '.C', '.CC', '.C++', '.ii'
]
OBJC_ENDINGS = ['.m', '.mi']
...
DEFAULT_ASYNCIFY_IMPORTS = [
    ...
    'emscripten_idb_load', 'emscripten_idb_store', 'emscripten_idb_delete',
    'emscripten_idb_exists', 'emscripten_idb_load_blob',
    'emscripten_idb_store_blob', 'SDL_Delay', 'emscripten_scan_registers',
    'emscripten_lazy_load_code', 'emscripten_fiber_swap',
    'wasi_snapshot_preview1.fd_sync', '__wasi_fd_sync', '_emval_await',
    '_dlopen_js', '__asyncjs__*'
]

DEFAULT_ASYNCIFY_EXPORTS = ['main', '__main_argc_argv']
...
def create_reproduce_file(name, args):
  ...
  with tarfile.open(name, 'w') as reproduce_file:
    reproduce_file.add(shared.path_from_root('emscripten-version.txt'),
...
def embed_memfile():
  return (settings.SINGLE_FILE
          or (settings.MEM_INIT_METHOD == 0 and
              (not settings.MAIN_MODULE and not settings.SIDE_MODULE
               and not settings.GENERATE_SOURCE_MAP)))
...
def expand_byte_size_suffixes(value):
  ...
  if not match:
    exit_with_error(
        "invalid byte size `%s`.  Valid suffixes are: kb, mb, gb, tb" % value)
...
def apply_user_settings():
  ...
  for key, value in user_settings.items():
    if key in settings.internal_settings:
      exit_with_error(
          '%s is an internal setting and cannot be set from command line', key)
    ...
    if value and value[0] == '@':
      ...
      if not os.path.exists(filename):
        exit_with_error('%s: file not found parsing argument: %s=%s' %
                        (filename, key, value))
    ...
    if filename and expected_type == list and value.strip()[0] != '[':
      ...
      try:
        ...
        exit_with_error(
            'a problem occurred in evaluating the content after a "-s", specifically "%s=%s": %s',
            key, value, str(e))
...
def ensure_archive_index(archive_file):
  ...
  if is_ar_file_with_missing_index(archive_file):
    diagnostics.warning(
        'emcc',
        '%s: archive is missing an index; Use emar when creating libraries to ensure an index is created',
        archive_file)
    ...
    if using_lld:
      for flag, takes_arg in UNSUPPORTED_LLD_FLAGS.items():
        ...
        if f.startswith(flag) or f.startswith('-' + flag):
          diagnostics.warning('linkflags',
                              'ignoring unsupported linker flag: `%s`', f)
...
def cxx_to_c_compiler(cxx):
  ...
  basename = basename.replace('clang++',
                              'clang').replace('g++',
                                               'gcc').replace('em++', 'emcc')
...
def get_binaryen_passes():
  ...
  if not feature_matrix.caniuse(feature_matrix.Feature.SIGN_EXT):
    logger.debug(
        'lowering sign-ext feature due to incompatiable target browser engines'
    )
  ...
  if optimizing:
    passes += [
        building.opt_level_to_str(settings.OPT_LEVEL, settings.SHRINK_LEVEL)
    ]
  ...
  if settings.ASYNCIFY == 1:
    ...
    passes += [
        '--pass-arg=asyncify-imports@%s' % ','.join(settings.ASYNCIFY_IMPORTS)
    ]
    ...
    def check_human_readable_list(items):
      for item in items:
        if item.count('(') != item.count(')'):
          logger.warning(
              'emcc: ASYNCIFY list contains an item without balanced parentheses ("(", ")"):'
          )
          logger.warning('   ' + item)
          logger.warning(
              'This may indicate improper escaping that led to splitting inside your names.'
          )
          logger.warning(
              'Try using a response file. e.g: -sASYNCIFY_ONLY=@funcs.txt. The format is a simple'
          )
    ...
    if settings.ASYNCIFY_REMOVE:
      ...
      passes += [
          '--pass-arg=asyncify-removelist@%s' %
          ','.join(settings.ASYNCIFY_REMOVE)
      ]
    if settings.ASYNCIFY_ADD:
      check_human_readable_list(settings.ASYNCIFY_ADD)
      passes += [
          '--pass-arg=asyncify-addlist@%s' % ','.join(settings.ASYNCIFY_ADD)
      ]
    if settings.ASYNCIFY_ONLY:
      check_human_readable_list(settings.ASYNCIFY_ONLY)
      passes += [
          '--pass-arg=asyncify-onlylist@%s' % ','.join(settings.ASYNCIFY_ONLY)
      ]
  elif settings.ASYNCIFY == 2:
    passes += ['--jspi']
    passes += [
        '--pass-arg=jspi-imports@%s' % ','.join(settings.ASYNCIFY_IMPORTS)
    ]
    passes += [
        '--pass-arg=jspi-exports@%s' % ','.join(settings.ASYNCIFY_EXPORTS)
    ]
...
def make_js_executable(script):
  ...
  try:
    os.chmod(script,
             stat.S_IMODE(os.stat(script).st_mode)
...
def filter_out_dynamic_libs(options, inputs):
  ...
  def check(input_file):
    if get_file_suffix(
        input_file
    ) in DYNAMICLIB_ENDINGS and not building.is_wasm_dylib(input_file):
...
def process_dynamic_libs(dylibs, lib_dirs):
  ...
  while to_process:
    ...
    for needed in dylink.needed:
      ...
      if path:
        ...
        exit_with_error(
            f'{os.path.normpath(dylib)}: shared library dependency not found: `{needed}`'
        )
  ...
  for dylib in dylibs:
    ...
    imports = [
        i.field for i in imports if i.kind in (webassembly.ExternType.FUNC,
...
def emsdk_cflags(user_args):
  ...
  if array_contains_any_of(user_args,
                           SIMD_INTEL_FEATURE_TOWER) or array_contains_any_of(
                               user_args, SIMD_NEON_FLAGS):
  ...
  return cflags + [
      '-Xclang', '-iwithsysroot' + os.path.join('/include', 'compat')
  ]
...
def get_cflags(user_args, is_cxx):
  ...
  if settings.USE_SHARED_HEAP:
    cflags.append('-D__EMSCRIPTEN_USE_SHARED_HEAP__=1')
...
def package_files(options, target):
  ...
  cmd = [shared.FILE_PACKAGER,
         shared.replace_suffix(target, '.data')] + file_args
  if options.preload_files:
    ...
    options.pre_js = js_manipulation.add_files_pre_js(options.pre_js,
                                                      file_code)
  else:
...
def run(args):
  ...
  if len(args) == 2 and args[1] == '-v':
    ...
    return shared.check_call([clang, '-v'] + get_target_flags(),
...
''')
  ...
  if '--cflags' in args:
    ...
    with misc_temp_files.get_file(suffix='.o') as temp_target:
      ...
      cmd = [
          compiler,
          utils.path_from_root('test', input_file), '-v', '-c', '-o',
          temp_target
      ] + args
      ...
      lines = [
          x for x in proc.stderr.splitlines() if clang in x and input_file in x
      ]
      parts = shlex.split(lines[0].replace('\\', '\\\\'))
      parts = [
          x for x in parts if x not in ['-c', '-o', '-v', '-emit-llvm']
          and input_file not in x and temp_target not in x
      ]
      print(shared.shlex_join(parts[1:]))
    return 0

  passthrough_flags = ['-print-search-dirs', '-print-libgcc-file-name']
  if any(a in args for a in passthrough_flags) or any(
      a.startswith('-print-file-name=') for a in args):
    return run_process([clang] + args + get_cflags(args, run_via_emxx),
                       check=False).returncode
  ...
  if 'EMMAKEN_CFLAGS' in os.environ:
    exit_with_error(
        '`EMMAKEN_CFLAGS` is no longer supported, please use `EMCC_CFLAGS` instead'
    )
  ...
  if state.mode != Mode.COMPILE_AND_LINK:
    ...
    for flag in state.link_flags:
      diagnostics.warning('unused-command-line-argument',
                          "argument unused during compilation: '%s'" % flag[1])
    for f in linker_inputs:
      diagnostics.warning(
          'unused-command-line-argument',
          "%s: linker input file unused because linking not done" % f[1])
  ...
  linker_arguments = phase_calculate_linker_inputs(options, state,
                                                   linker_inputs)
  ...
  phase_calculate_system_libraries(state, linker_arguments, linker_inputs,
                                   newargs)
...
def phase_parse_arguments(state):
  ...
  if options.post_link or options.oformat == OFormat.BARE:
    diagnostics.warning(
        'experimental',
        '--oformat=bare/--post-link are experimental and subject to change.')
...
def phase_setup(options, state, newargs):
  ...
  if settings.RUNTIME_LINKED_LIBS:
    diagnostics.warning(
        'deprecated',
        'RUNTIME_LINKED_LIBS is deprecated; you can simply list the libraries directly on the commandline now'
    )
  ...
  for i in range(len(newargs)):
    ...
    if arg in ('-MT', '-MF', '-MJ', '-MQ', '-D', '-U', '-o', '-x',
               ...
               '-imultilib', '-A', '-isystem', '-iquote', '-install_name',
               '-compatibility_version', '-current_version', '-I', '-L',
               '-include-pch', '-Xlinker', '-Xclang'):
      skip = True
    ...
    if not arg.startswith('-'):
      ...
      if file_suffix in DYNAMICLIB_ENDINGS and not building.is_bitcode(
          arg) and not building.is_wasm(arg):
        ...
        diagnostics.warning(
            ...
            f'unrecognized file type: `{arg}`.  Mapping to `{flag}` and hoping for the best'
        )
      ...
      for flag_index, flag in enumerate(link_flags_to_add):
        add_link_flag(state, i + float(flag_index) / len(link_flags_to_add),
                      flag)
      newargs[i] = ''
    elif arg == '-Xlinker':
  ...
  newargs = [
      x for x in newargs
      if x not in SIMD_INTEL_FEATURE_TOWER and x not in SIMD_NEON_FLAGS
  ]
  ...
  if state.mode in (Mode.COMPILE_ONLY, Mode.PREPROCESS_ONLY):
    ...
    if options.output_file and len(input_files) > 1:
      exit_with_error(
          'cannot specify -o with -c/-S/-E/-M and multiple source files')
  else:
    for arg in state.orig_args:
  ...
  if settings.USE_SHARED_HEAP:
    settings.USE_RELOCATION_OFFSET = 1

  if settings.USE_RELOCATION_OFFSET:
    settings.RELOCATABLE = 1
  ...
  if 'DISABLE_EXCEPTION_CATCHING' in user_settings and 'EXCEPTION_CATCHING_ALLOWED' in user_settings:
    ...
    if user_settings['DISABLE_EXCEPTION_CATCHING'] in ('0', '2'):
      diagnostics.warning(
          'deprecated',
          'DISABLE_EXCEPTION_CATCHING=X is no longer needed when specifying EXCEPTION_CATCHING_ALLOWED'
      )
    else:
      exit_with_error(
          'DISABLE_EXCEPTION_CATCHING and EXCEPTION_CATCHING_ALLOWED are mutually exclusive'
      )
  ...
  if settings.WASM_EXCEPTIONS:
    if 'DISABLE_EXCEPTION_CATCHING' in user_settings:
      exit_with_error(
          'DISABLE_EXCEPTION_CATCHING is not compatible with -fwasm-exceptions'
      )
    if 'DISABLE_EXCEPTION_THROWING' in user_settings:
      exit_with_error(
          'DISABLE_EXCEPTION_THROWING is not compatible with -fwasm-exceptions'
      )
  ...
  if settings.MEMORY64:
    diagnostics.warning(
        'experimental',
        '-sMEMORY64 is still experimental. Many features may not work.')
  ...
  if settings.SUPPORT_LONGJMP == 'wasm':
    ...
    if not settings.DISABLE_EXCEPTION_THROWING:
      exit_with_error(
          'SUPPORT_LONGJMP=wasm cannot be used with DISABLE_EXCEPTION_CATCHING=0'
      )
...
def setup_pthreads(target):
  if settings.RELOCATABLE:
    ...
    if settings.SIDE_MODULE:
      diagnostics.warning('experimental',
                          '-sSIDE_MODULE + pthreads is experimental')
    elif settings.MAIN_MODULE:
      diagnostics.warning('experimental',
                          '-sMAIN_MODULE + pthreads is experimental')
    elif settings.LINKABLE:
      diagnostics.warning('experimental',
                          '-sLINKABLE + pthreads is experimental')
  ...
  if settings.MAIN_MODULE:
    settings.REQUIRED_EXPORTS += [
        '_emscripten_thread_sync_code', '__dl_seterr'
    ]
  ...
  if not settings.MINIMAL_RUNTIME:
    ...
    settings.EXPORTED_RUNTIME_METHODS += [
        'keepRuntimeAlive', 'ExitStatus', 'wasmMemory'
    ]
...
def phase_linker_setup(options, state, newargs):
  autoconf = os.environ.get(
      'EMMAKEN_JUST_CONFIGURE') or 'conftest.c' in state.orig_args
  if autoconf:
  ...
  if not settings.RUNTIME_DEBUG:
    settings.RUNTIME_DEBUG = (settings.LIBRARY_DEBUG or settings.GL_DEBUG
                              or settings.DYLINK_DEBUG or settings.OPENAL_DEBUG
                              or settings.SYSCALL_DEBUG
                              or settings.WEBSOCKET_DEBUG
                              or settings.SOCKET_DEBUG or settings.FETCH_DEBUG
  ...
  if options.output_file:
    ...
    if dirname and not os.path.isdir(dirname):
      exit_with_error(
          "specified output file (%s) is in a directory that does not exist" %
          target)
  elif autoconf:
  ...
  if settings.EXTRA_EXPORTED_RUNTIME_METHODS:
    diagnostics.warning(
        'deprecated',
        'EXTRA_EXPORTED_RUNTIME_METHODS is deprecated, please use EXPORTED_RUNTIME_METHODS instead'
    )
  ...
  if not options.oformat and (options.relocatable or
                              (options.shared and not settings.SIDE_MODULE)):
  ...
  if (options.oformat == OFormat.WASM
      or settings.PURE_WASI) and not settings.SIDE_MODULE:
  ...
  if options.no_entry:
    ...
    if '_main' in settings.EXPORTED_FUNCTIONS:
      ...
      logger.debug(
          'including `_main` in EXPORTED_FUNCTIONS is not necessary in standalone mode'
      )
  else:
  ...
  if settings.STANDALONE_WASM:
    ...
    if 'EXIT_RUNTIME' in user_settings:
      exit_with_error(
          'Explictly setting EXIT_RUNTIME not compatible with STANDALONE_WASM.  EXIT_RUNTIME will always be True for programs (with a main function) and False for reactors (not main function).'
      )
  ...
  if settings.ASSERTIONS:
    ...
    if not settings.PURE_WASI and '-nostdlib' not in newargs and '-nodefaultlibs' not in newargs:
      default_setting('STACK_OVERFLOW_CHECK',
                      max(settings.ASSERTIONS, settings.STACK_OVERFLOW_CHECK))
  ...
  if settings.WASM == 2 and settings.SINGLE_FILE:
    exit_with_error(
        'cannot have both WASM=2 and SINGLE_FILE enabled at the same time')

  if settings.SEPARATE_DWARF and settings.WASM2JS:
    exit_with_error(
        'cannot have both SEPARATE_DWARF and WASM2JS at the same time (as there is no wasm file)'
    )
  ...
  if 'CLOSURE_WARNINGS' in user_settings:
    ...
    diagnostics.warning(
        'deprecated',
        'CLOSURE_WARNINGS is deprecated, use -Wclosure/-Wno-closure instread')
  ...
  if settings.RELOCATABLE:
    ...
    if settings.MINIMAL_RUNTIME:
      exit_with_error(
          'MINIMAL_RUNTIME is not compatible with relocatable output')
    if settings.WASM2JS:
  ...
  if settings.MEMORYPROFILER:
    settings.REQUIRED_EXPORTS += [
        '__heap_base', 'emscripten_stack_get_base', 'emscripten_stack_get_end',
  ...
  settings.ASYNCIFY_REMOVE = unmangle_symbols_from_cmdline(
      settings.ASYNCIFY_REMOVE)
  settings.ASYNCIFY_ONLY = unmangle_symbols_from_cmdline(
  ...
  if options.oformat != OFormat.OBJECT and final_suffix in (
      '.o', '.bc', '.so', '.dylib') and not settings.SIDE_MODULE:
    diagnostics.warning(
        'emcc',
  ...
  if settings.SUPPORT_BIG_ENDIAN:
    settings.DEFAULT_LIBRARY_FUNCS_TO_INCLUDE += [
        '$LE_HEAP_STORE_U16', '$LE_HEAP_STORE_I16', '$LE_HEAP_STORE_U32',
        '$LE_HEAP_STORE_I32', '$LE_HEAP_STORE_F32', '$LE_HEAP_STORE_F64',
        '$LE_HEAP_LOAD_U16', '$LE_HEAP_LOAD_I16', '$LE_HEAP_LOAD_U32',
        '$LE_HEAP_LOAD_I32', '$LE_HEAP_LOAD_F32', '$LE_HEAP_LOAD_F64'
    ]
  ...
  if options.use_closure_compiler != 0:
    ...
    settings.TRANSPILE_TO_ES5 = (settings.MIN_EDGE_VERSION < 12
                                 or settings.MIN_FIREFOX_VERSION < 44
                                 or settings.MIN_CHROME_VERSION < 49
                                 or settings.MIN_SAFARI_VERSION < 110000
                                 or settings.MIN_IE_VERSION != 0x7FFFFFFF)

    if options.use_closure_compiler is None and settings.TRANSPILE_TO_ES5:
      diagnostics.warning(
          'transpile',
          'enabling transpilation via closure due to browser version settings.  This warning can be suppressed by passing `--closure=1` or `--closure=0` to opt into our explicitly.'
      )
  ...
  if settings.MIN_WEBGL_VERSION > settings.MAX_WEBGL_VERSION:
    exit_with_error(
        'MIN_WEBGL_VERSION must be smaller or equal to MAX_WEBGL_VERSION!')
  ...
  if options.use_preload_plugins or len(options.preload_files) or len(
      options.embed_files):
  ...
  if not settings.BOOTSTRAPPING_STRUCT_INFO:
    ...
    if not settings.STANDALONE_WASM and (settings.EXIT_RUNTIME
  ...
  if settings.SAFE_HEAP:
    ...
    settings.REQUIRED_EXPORTS += [
        'emscripten_get_sbrk_ptr', 'emscripten_stack_get_base'
    ]
  ...
  if settings.WASM_WORKERS:
    ...
    settings.DEFAULT_LIBRARY_FUNCS_TO_INCLUDE += [
        '_wasm_worker_initializeRuntime'
    ]
    # set location of Wasm Worker bootstrap JS file
    if settings.WASM_WORKERS == 1:
      settings.WASM_WORKER_FILE = unsuffixed(
          os.path.basename(target)) + '.ww.js'
    settings.JS_LIBRARIES.append(
        (0, shared.path_from_root('src', 'library_wasm_worker.js')))
  ...
  if settings.FORCE_FILESYSTEM and not settings.MINIMAL_RUNTIME:
    ...
    if not settings.WASMFS:
      ...
      settings.EXPORTED_RUNTIME_METHODS += [
          'FS_createLazyFile', 'FS_createDevice'
      ]
  ...
  def check_memory_setting(setting):
    if settings[setting] % webassembly.WASM_PAGE_SIZE != 0:
      exit_with_error(
          f'{setting} must be a multiple of WebAssembly page size (64KiB), was {settings[setting]}'
      )
  ...
  if 'MAXIMUM_MEMORY' in user_settings and not settings.ALLOW_MEMORY_GROWTH:
    diagnostics.warning(
        'unused-command-line-argument',
        'MAXIMUM_MEMORY is only meaningful with ALLOW_MEMORY_GROWTH')
  ...
  if settings.EXPORT_ES6:
    if not settings.MODULARIZE:
      ...
      settings.MODULARIZE = 1
    if shared.target_environment_may_be(
        'node') and not settings.USE_ES6_IMPORT_META:
  ...
  if settings.MODULARIZE and not settings.DECLARE_ASM_MODULE_EXPORTS:
    ...
    exit_with_error(
        'DECLARE_ASM_MODULE_EXPORTS=0 is not compatible with MODULARIZE')
  ...
  if not settings.DECLARE_ASM_MODULE_EXPORTS or '-lexports.js' in [
      x for _, x in state.link_flags
  ]:
  ...
  if settings.MINIMAL_RUNTIME:
    ...
    if options.shell_path == utils.path_from_root('src/shell.html'):
      options.shell_path = utils.path_from_root(
  ...
  if settings.MEMORY64:
    ...
    if not settings.DISABLE_EXCEPTION_CATCHING:
      exit_with_error(
          'MEMORY64 is not compatible with DISABLE_EXCEPTION_CATCHING=0')
    settings.WASM_BIGINT = 1
  ...
  if settings.EVAL_CTORS:
    if settings.WASM2JS:
      ...
      exit_with_error(
          'EVAL_CTORS is not compatible with relocatable yet (movable segments)'
      )
  ...
  if (settings.MAYBE_WASM2JS or settings.AUTODEBUG or settings.LINKABLE
      ...
      or (settings.MAIN_MODULE == 1 and
          (settings.DYNCALLS or not settings.WASM_BIGINT))):
  ...
  if ('leak' in sanitize
      or 'address' in sanitize) and not settings.ALLOW_MEMORY_GROWTH:
  ...
  if settings.USE_OFFSET_CONVERTER and settings.WASM2JS:
    exit_with_error(
        'wasm2js is not compatible with USE_OFFSET_CONVERTER (see #14630)')
  ...
  if 'address' in sanitize:
    ...
    if settings.ASAN_SHADOW_SIZE != -1:
      diagnostics.warning(
          'emcc',
          'ASAN_SHADOW_SIZE is ignored and will be removed in a future release'
      )
  ...
  if settings.MINIMAL_RUNTIME:
    if settings.EXIT_RUNTIME:
      settings.DEFAULT_LIBRARY_FUNCS_TO_INCLUDE += [
          'proc_exit', '$callRuntimeCallbacks'
      ]
  else:
  ...
  if settings.ASYNCIFY:
    ...
    settings.ASYNCIFY_IMPORTS = [
        get_full_import_name(i) for i in settings.ASYNCIFY_IMPORTS
    ]

    if settings.ASYNCIFY == 2:
      diagnostics.warning('experimental',
                          'ASYNCIFY with stack switching is experimental')

  if settings.WASM2JS:
    if settings.GENERATE_SOURCE_MAP:
      exit_with_error(
          'wasm2js does not support source maps yet (debug in wasm for now)')
    if settings.WASM_BIGINT:
  ...
  if settings.NODE_CODE_CACHING:
    if settings.WASM_ASYNC_COMPILATION:
      exit_with_error(
          'NODE_CODE_CACHING requires sync compilation (WASM_ASYNC_COMPILATION=0)'
      )
    if not shared.target_environment_may_be('node'):
      exit_with_error(
          'NODE_CODE_CACHING only works in node, but target environments do not include it'
      )
    if settings.SINGLE_FILE:
      exit_with_error(
          'NODE_CODE_CACHING saves a file on the side and is not compatible with SINGLE_FILE'
      )

  if not js_manipulation.isidentifier(settings.EXPORT_NAME):
    exit_with_error(
        f'EXPORT_NAME is not a valid JS identifier: `{settings.EXPORT_NAME}`')
  ...
  if settings.EMSCRIPTEN_TRACING and settings.ALLOW_MEMORY_GROWTH:
    ...
    settings.REQUIRED_EXPORTS += [
        'emscripten_stack_get_current', 'emscripten_stack_get_base',
        'emscripten_stack_get_end'
    ]
  ...
  settings.LINK_AS_CXX = (
      run_via_emxx or settings.DEFAULT_TO_CXX) and '-nostdlib++' not in newargs
  ...
  if not settings.LINK_AS_CXX:
    ...
    for setting in cxx_only_settings:
      if setting in user_settings:
        diagnostics.warning(
            'linkflags',
            'setting `%s` is not meaningful unless linking as C++', setting)
  ...
  if settings.EXPORT_EXCEPTION_HANDLING_HELPERS:
    ...
    settings.EXPORTED_FUNCTIONS += [
        'getExceptionMessage', '___get_exception_message', '_free'
    ]
    if settings.WASM_EXCEPTIONS:
      settings.EXPORTED_FUNCTIONS += [
...
def phase_compile_inputs(options, state, newargs, input_files):
  ...
  def get_clang_command(src_file):
    return get_compiler(src_file) + get_cflags(
        state.orig_args, use_cxx(src_file)) + compile_args + [src_file]

  def get_clang_command_preprocessed(src_file):
    return get_compiler(src_file) + get_clang_flags(
        state.orig_args) + compile_args + [src_file]
  ...
  if state.mode == Mode.PREPROCESS_ONLY:
    for input_file in [x[1] for x in input_files]:
      ...
      logger.debug(('just preprocessor ' if state.
                    has_dash_E else 'just dependencies: ') + ' '.join(cmd))
      shared.check_call(cmd)
    return []
  ...
  if state.mode == Mode.PCH:
    ...
    for header in headers:
      if not shared.suffix(header) in HEADER_ENDINGS:
        exit_with_error(
            f'cannot mix precompiled headers with non-header inputs: {headers} : {header}'
        )
      cmd = get_clang_command(header)
      if options.output_file:
        cmd += ['-o', options.output_file]
      logger.debug(
          f"running (for precompiled headers): {cmd[0]} {' '.join(cmd[1:])}")
      shared.check_call(cmd)
      return []
  ...
  def get_object_filename(input_file):
    if state.mode == Mode.COMPILE_ONLY:
      ...
      if options.output_file:
        ...
        if get_file_suffix(
            options.output_file
        ) == '.bc' and not settings.LTO and '-emit-llvm' not in state.orig_args:
          diagnostics.warning(
              'emcc',
        ...
        return unsuffixed_basename(
            input_file) + options.default_object_extension
    else:
      return in_temp(
          unsuffixed(uniquename(input_file)) +
          options.default_object_extension)
  ...
  def compile_source_file(i, input_file):
    ...
    if state.mode == Mode.COMPILE_AND_LINK and '-gsplit-dwarf' in newargs:
      ...
      cmd += [
          '-Xclang', '-split-dwarf-file', '-Xclang',
          unsuffixed_basename(input_file) + '.dwo'
      ]
      cmd += [
          '-Xclang', '-split-dwarf-output', '-Xclang',
          unsuffixed_basename(input_file) + '.dwo'
      ]
    shared.check_call(cmd)
  ...
  for i, input_file in input_files:
    ...
    if file_suffix in SOURCE_ENDINGS + ASSEMBLY_ENDINGS or (
        state.has_dash_c and file_suffix == '.bc'):
...
def phase_calculate_system_libraries(state, linker_arguments, linker_inputs,
                                     newargs):
  extra_files_to_link = []
  ...
  all_linker_inputs = [f
                       for _, f in sorted(linker_inputs)] + extra_files_to_link
  extra_files_to_link += system_libs.calculate(all_linker_inputs,
                                               newargs,
                                               forced=state.forced_stdlibs)
...
def phase_source_transforms(options):
  ...
  if options.js_transform:
    ...
    shared.check_call(
        building.remove_quotes(
            shlex.split(options.js_transform, posix=posix) +
            [os.path.abspath(final_js)]))
...
def phase_memory_initializer(memfile):
  ...
  src = read_file(final_js)
  src = do_replace(src, '// {{MEM_INITIALIZER}}',
...
def phase_final_emitting(options, state, target, wasm_target, memfile):
  ...
  if settings.USE_PTHREADS:
    ...
    contents = shared.read_and_preprocess(
        utils.path_from_root('src/worker.js'), expand_macros=True)
    write_file(worker_output, contents)

    # Minify the worker.js file in optimized builds
    if (settings.OPT_LEVEL >= 1
        or settings.SHRINK_LEVEL >= 1) and not settings.DEBUG_LEVEL:
      minified_worker = building.acorn_optimizer(worker_output,
                                                 ['minifyWhitespace'],
                                                 return_output=True)
  ...
  if settings.WASM_WORKERS == 1:
    ...
    contents = shared.read_and_preprocess(
        shared.path_from_root('src/wasm_worker.js'), expand_macros=True)
    write_file(worker_output, contents)
  ...
  if settings.MINIMAL_RUNTIME == 2 and settings.USE_CLOSURE_COMPILER and settings.DEBUG_LEVEL == 0:
    shared.run_js_tool(utils.path_from_root('tools/unsafe_optimizations.js'),
                       [final_js, '-o', final_js],
    ...
    # mode)
    final_js = building.closure_compiler(
        final_js,
        pretty=False,
        advanced=False,
  ...
  if settings.EXPORT_ES6 and settings.USE_ES6_IMPORT_META:
    ...
    write_file(
        final_js,
        src.replace('EMSCRIPTEN$IMPORT$META',
                    'import.meta').replace('EMSCRIPTEN$AWAIT$IMPORT',
                                           'await import'))
  ...
  if settings.SPLIT_MODULE:
    diagnostics.warning(
        'experimental',
        'The SPLIT_MODULE setting is experimental and subject to change')
    do_split_module(wasm_target)

  for f in generated_text_files_with_native_eols:
    tools.line_endings.convert_line_endings_in_file(f, os.linesep,
                                                    options.output_eol)
...
def version_string():
  ...
  if os.path.exists(utils.path_from_root('.git')):
    git_rev = run_process(['git', 'rev-parse', 'HEAD'],
                          stdout=PIPE,
                          stderr=PIPE,
                          cwd=utils.path_from_root()).stdout.strip()
  ...
  for i in range(len(newargs)):
    ...
    if arg.startswith('-O'):
      ...
      if requested_level == 's':
        ...
        settings.SHRINK_LEVEL = 0
      settings.OPT_LEVEL = validate_arg_level(requested_level,
                                              3,
      ...
      diagnostics.warning(
          'deprecated',
          '--llvm-opts is deprecated.  All non-emcc args are passed through to clang.'
      )
      ...
      if fmt not in formats:
        exit_with_error('invalid output format: `%s` (must be one of %s)' %
                        (fmt, formats))
      ...
      if arg != '0':
        exit_with_error(
            '0 is the only supported option for --minify; 1 has been deprecated'
        )
      ...
      if is_int(requested_level):
        ...
        settings.DEBUG_LEVEL = validate_arg_level(
            requested_level, 4, 'invalid debug level: ' + arg)
        ...
        if settings.DEBUG_LEVEL < 3:
          ...
          if settings.DEBUG_LEVEL == 4:
            ...
            diagnostics.warning('deprecated',
                                'please replace -g4 with -gsource-map')
      else:
        if requested_level.startswith('force_dwarf'):
          exit_with_error(
              'gforce_dwarf was a temporary option and is no longer necessary (use -g)'
          )
          ...
          if requested_level != 'separate-dwarf':
            if not requested_level.startswith(
                'separate-dwarf=') or requested_level.count('=') != 1:
      ...
      logger.info('clearing cache as requested by --clear-cache: `%s`',
                  cache.cachedir)
      cache.erase()
      ...
      exit_with_error(
          'cannot --separate-asm with the wasm backend, since not emitting asm.js'
      )
...
def phase_binaryen(target, options, wasm_target):
  ...
  if settings.GENERATE_SOURCE_MAP and not settings.SOURCE_MAP_BASE:
    logger.warning(
        "Wasm source map won't be usable in a browser without --source-map-base"
    )
    ...
    if settings.OPT_LEVEL >= 2 and settings.DEBUG_LEVEL <= 2:
      ...
      with ToolchainProfiler.profile_block('minify_wasm'):
        ...
        final_js = building.minify_wasm_js(
            ...
            and not options.use_closure_compiler,
            debug_info=intermediate_debug_info)
        save_intermediate_with_wasm('postclean', wasm_target)

  if settings.ASYNCIFY_LAZY_LOAD_CODE:
    with ToolchainProfiler.profile_block('asyncify_lazy_load_code'):
      building.asyncify_lazy_load_code(wasm_target,
                                       debug=intermediate_debug_info)

  def preprocess_wasm2js_script():
    return read_and_preprocess(utils.path_from_root('src/wasm2js.js'),
                               expand_macros=True)
  ...
  if settings.WASM2JS:
    ...
    if settings.WASM == 2:
      ...
      if options.emit_symbol_map:
        symbols_file_js = shared.replace_or_append_suffix(
            wasm2js_template, '.symbols')
    else:
  ...
  if options.emit_symbol_map:
    ...
    if os.path.exists(wasm_target):
      with ToolchainProfiler.profile_block('handle_final_symbols'):
        building.handle_final_wasm_symbols(wasm_file=wasm_target,
                                           symbols_file=symbols_file,
                                           debug_info=intermediate_debug_info)
        save_intermediate_with_wasm('symbolmap', wasm_target)

  if settings.DEBUG_LEVEL >= 3 and settings.SEPARATE_DWARF and os.path.exists(
      wasm_target):
  ...
  if final_js and settings.SINGLE_FILE and not settings.WASM2JS:
    ...
    if settings.MINIMAL_RUNTIME:
      js = do_replace(js, '<<< WASM_BINARY_DATA >>>',
                      base64_encode(read_binary(wasm_target)))
    else:
      js = do_replace(js, '<<< WASM_BINARY_FILE >>>',
                      get_subresource_location(wasm_target))
    delete_file(wasm_target)
...
def module_export_name_substitution():
  global final_js
  logger.debug(
      f'Private module export name substitution with {settings.EXPORT_NAME}')
  src = read_file(final_js)
  ...
  src = re.sub(
      r'{\s*[\'"]?__EMSCRIPTEN_PRIVATE_MODULE_EXPORT_NAME_SUBSTITUTION__[\'"]?:\s*1\s*}',
      replacement, src)
  write_file(final_js, src)
  shared.get_temp_files().note(final_js)
...
def generate_traditional_runtime_html(target, options, js_target,
                                      target_basename, wasm_target, memfile):
  script = ScriptSource()
  ...
  if settings.PROXY_TO_WORKER:
    proxy_worker_filename = (settings.PROXY_TO_WORKER_FILENAME
                             or target_basename) + '.js'
...
def minify_html(filename):
  ...
  if settings.DEBUG_LEVEL == 0:
    opts += [
        '--collapse-whitespace', '--collapse-inline-tag-whitespace',
        '--remove-comments', '--remove-tag-whitespace', '--sort-attributes',
        '--sort-class-name'
    ]
  # -g2 and greater do not minify HTML at all
  if settings.DEBUG_LEVEL <= 1:
    opts += [
        '--decode-entities', '--collapse-boolean-attributes',
        '--remove-attribute-quotes', '--remove-redundant-attributes',
  ...
  shared.check_call(shared.get_npm_cmd('html-minifier-terser') +
                    [filename, '-o', filename] + opts,
...
def generate_html(target, options, js_target, target_basename, wasm_target,
  ...
  if settings.MINIFY_HTML and (settings.OPT_LEVEL >= 1
                               or settings.SHRINK_LEVEL >= 1):
    minify_html(target)
...
def worker_js_script(proxy_worker_filename):
  ...
  proxy_client_src = do_replace(proxy_client_src, '{{{ filename }}}',
                                proxy_worker_filename)
  proxy_client_src = do_replace(proxy_client_src, '{{{ IDBStore.js }}}',
                                idb_store_src)
...
def parse_value(text, expected_type):
  ...
  def parse_string_value(text):
    ...
    if first == "'" or first == '"':
      ...
      assert text[-1] == text[0] and len(
          text
      ) > 1, 'unclosed opened quoted string. expected final character to be "%s" and length to be greater than 1 in "%s"' % (
          text[0], text)
      return text[1:-1]
    return text
    ...
    while True:
      ...
      if not (first == "'" or first == '"'):
        ...
        while True:  # Continue until closing quote found
          if index >= len(values):
            exit_with_error(
                "unclosed quoted string. expected final character to be '%s' in '%s'"
                % (first, values[start]))
  ...
  def parse_string_list(text):
    ...
    if text and text[0] == '[':
      if text[-1] != ']':
        exit_with_error(
            'unclosed opened string list. expected final character to be "]" in "%s"'
            % (text))
      text = text[1:-1]
    if text.strip() == "":
      return []
  ...
  if clamp:
    if level > max_level:
      logger.warning("optimization level '-O" + level_string +
                     "' is not supported; using '-O" + str(max_level) +
                     "' instead")
      level = max_level
